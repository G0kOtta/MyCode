...
class Multiply < Struct.new(:left, :right)
  ...
  def reduce(enviroment)
    if left.reducible?
      Multiply.new(left.reducible(enviroment), right)
    elsif right.reducible?
      Multiply.new(left, right.reducible(enviroment))
    else
      Number.new(left.value * right.value)
    end
  end
...
class LessThan <Struct.new(:left, :right)
  ...
  def reduce(enviroment)
    if left.reducible?
      LessThan.new(left.reducible(enviroment), right)
    elsif right.reducible?
      LessThan.new(left, right.reducible(enviroment))
    else
